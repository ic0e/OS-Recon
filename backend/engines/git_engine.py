...
CRITICAL_KEYWORDS = re.compile(
    r"(key|secret|token|config|db|passwd|auth|credential|private|env)", re.IGNORECASE
)
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            for repo in repos:
                ...
                # "POINTS OF INTEREST"
                has_sensitive_name = bool(
                    CRITICAL_KEYWORDS.search(repo_name)
                    or CRITICAL_KEYWORDS.search(description)
                )
                ...
                if has_sensitive_name:
                    repo_summary["reasons"].append(
                        "Contains security-sensitive keywords in metadata."
                    )
                if is_popular:
                    repo_summary["reasons"].append(
                        f"High traction asset ({stars} stars)."
                    )
                if not is_fork and repo_name.lower() in [
                    username.lower(),
                    "dotfiles",
                    "vault",
                ]:
                    repo_summary["reasons"].append(
                        "Potential personal configuration hub or profile vault."
                    )
...
async def fetch_repo_commits(username: str, repo_name: str):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 404:
                return {
                    "error": f"Repository '{repo_name}' or user '{username}' not found."
                }
            elif response.status_code != 200:
                return {"error": f"API Error: {response.status_code}"}
...
async def extract_emails_from_commits(username: str, repos: list) -> list[str]:
    ...
    for commits in results:
        for item in commits:
            ...
            if (
                isinstance(author_login, dict)
                and author_login.get("login", "").lower() != username.lower()
            ):
                continue
